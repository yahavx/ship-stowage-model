...
Operations
RobustStowageAlgorithm::unloadContainer(ContainerShip &ship, Port &port, const ContainerPosition &containerPos,
...
RobustStowageAlgorithm::tryMovingContainer(ContainerShip &ship, Port &port, ContainerPosition &pos, std::vector<ContainerPosition> &containersToUnload) {
    auto posToMoveTo = findPosToMoveTo(ship, containersToUnload);
...
RobustStowageAlgorithm::findPosToMoveTo(ContainerShip &ship, std::vector<ContainerPosition> &containersToUnload) {
        ...
        for (int y = 0; y < dims.Y(); y++) {
            ...
            if (z >= 0) { // There is space to load on top in this x,y position
                ...
                if (!foundContainerToUnload) // There is no container to unload in this x,y position so return it
                    return Position(x, y, ship.getCargo().getAvailableFloorToLoadContainer(x, y));
